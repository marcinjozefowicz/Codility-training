...
public class Range {
    public static int[] solution(String S, int[] P, int[] Q) {
        ...
        for(int j=0;j<P.length;j++){
            ...
            for(int i=0;i<4;i++){
                if(Q[j]==P[j]) answerArray[j]=i+1;
                if(sumSuffix[i][Q[j]]-sumSuffix[i][P[j]]>0) {answerArray[j]=i+1;break;}
            }
    ...
    public static void main(String[] args) {
        System.out.println(Arrays.toString(Range.solution(new String("GACACCATA"),new int[]{0, 0, 4, 7},new int[]{8, 2, 5, 7})));
    }
}
